...
def simulation_init(xnum,ynum,znum,xlen,ylen,zlen,temp,dt,box):
    sumv=0
    sumv2=0
    n=0
    ...
    for i in range(xnum):
        for j in range(ynum):
            for k in range(znum):
                ...
                places[n][2]=k*zlen
                n+=1
    places=places-box*np.rint(places/box)
...
def intergrate(places,places_before,vels,forces,meane,delta,box):
    placediff_before=places-places_before # possiblity that place_next and place_before in different cube
    placediff_before=placediff_before-box*np.rint(placediff_before/box)
    places_next=places+placediff_before+delta**2*forces
...
def simulate(xnum,ynum,znum,xlen,ylen,zlen,temp,dt,box,rc,tnum,collectnum):
    pl,vs=simulation_init(xnum,ynum,znum,xlen,ylen,zlen,temp,dt,box)
    #print(pl)
    pl_m=pl-dt*vs
    ...
    for i in range(tnum):
        ...
        pl,pl_m,vs,temp,etot=intergrate(pl,pl_m,vs,forces,meane,dt,box)
        if i==0:
            print(forces)
            print("T:",temp)
            print("E:",etot)
...
if __name__=="__main__":
    ...
    plt.ylabel("g(r)")
    plt.plot(r,g)
    plt.savefig("crosstest_2.png")
    plt.clf() # clear figure

    plt.title("t vs T by MD simulation. (Box size 6.5, lattice size 1.3, N=125,T=12~15s)")
    plt.xlabel("t")
    plt.ylabel("T")
    plt.plot(np.array(range(200))*dt,T_collect)
    #plt.savefig("crosstest_2_T.png")
